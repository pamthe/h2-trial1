...
public class StudentJdbcRepository {
	...
	public int insert(Friend friend) {
		return jdbcTemplate.update("insert into friend (id, emailID, friendList) " + "values(?, ?, ?)",
				new Object[] { friend.getId(), friend.getEmailID(),friend.getFriendList() });
	}

	public int update(Friend friend) {
		return jdbcTemplate.update("update friend " + " set emailID = ? ,friendList=?" + " where id = ?",
				new Object[] {friend.getEmailID(), friend.getFriendList(),friend.getId(), });
	}
